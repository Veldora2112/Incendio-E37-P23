...
const float ESCALA_SENSOR = 100.0f;    // [grados C / V] sensibilidad nominal
...
const float M_CAL = 1.000f;            // ganancia  (adimensional)
const float B_CAL = 0.000f;            // offset    [grados C]
...
void loop() {
  ...
  if (ahora - t_ultima_muestra >= PERIODO_MS) {
    ...
    float valor    = cuentas_a_fisica(cuentas);        // conversion (item 3)
